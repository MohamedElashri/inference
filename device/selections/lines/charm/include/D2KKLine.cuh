...
namespace d2kk_line {
  ...
  struct d2kk_line_t : public SelectionAlgorithm, Parameters, CompositeParticleLine<d2kk_line_t, Parameters> {
    struct DeviceProperties {
      ...
      float minZ;
      bool OppositeSign;
      Allen::Monitoring::Histogram<>::DeviceType histogram_d02kk_mass;
      Allen::Monitoring::Histogram<>::DeviceType histogram_d02kk_pt;
      DeviceProperties(const d2kk_line_t& algo, const Allen::Context& ctx) :
        minComboPt(algo.m_minComboPt), maxVertexChi2(algo.m_maxVertexChi2), maxDOCA(algo.m_maxDOCA),
        minEta(algo.m_minEta), maxEta(algo.m_maxEta), minTrackPt(algo.m_minTrackPt), massWindow(algo.m_massWindow),
        minTrackIP(algo.m_minTrackIP), ctIPScale(algo.m_ctIPScale), minZ(algo.m_minZ),
        OppositeSign(algo.m_opposite_sign.value()), histogram_d02kk_mass(algo.m_histogram_d02kk_mass.data(ctx)),
        histogram_d02kk_pt(algo.m_histogram_d02kk_pt.data(ctx))
      {}
    };
