  ...
  std::tuple<const Allen::Views::Physics::CompositeParticle> input)
{
  ...
  const bool decision = vertex.pt() > properties.minComboPt && vertex.chi2() < properties.maxVertexChi2 &&
                        ...
                        fabsf(particle.m12(Allen::mPi, Allen::mPi) - Allen::mDz) < properties.massWindow &&
  ...
  bool sel)
{
  ...
  const auto vertex = particle.vertex();
  if (sel) {
    properties.histogram_d02pipi_mass.increment(particle.m12(Allen::mPi, Allen::mPi));
    properties.histogram_d02pipi_pt.increment(vertex.pt());
  }
}
