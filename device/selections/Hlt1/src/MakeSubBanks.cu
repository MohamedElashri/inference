    ...
    // Handle lines that select CompositeParticles.
      ...
      for (unsigned i_cand = 0; i_cand < n_cand; i_cand++) {
        ...
        const unsigned sv_index =
          sv_duplicate_map[selected_object_offset + i_sv] >= 0 ? sv_duplicate_map[selected_object_offset + i_sv] : i_sv;
        unsigned obj_index = 0;
...
__host__ __device__ void make_subbanks::make_rb_objtyp_bank(
...
__host__ __device__ void make_subbanks::make_rb_stdinfo_bank(
