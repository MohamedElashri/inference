...
\************************************************************************/
#include "ODINNZSFilter.cuh"
#include "Event/ODIN.h"
#include "ODINBank.cuh"

INSTANTIATE_ALGORITHM(odin_nzsfilter::odin_nzsfilter_t)

void odin_nzsfilter::odin_nzsfilter_t::set_arguments_size(
...
__global__ void
odin_nzsfilter_kernel(odin_nzsfilter::Parameters parameters, const unsigned number_of_events, const bool nzs_bit)
{
  for (unsigned idx = threadIdx.x; idx < number_of_events; idx += blockDim.x) {
    ...
    const bool nzst = static_cast<bool>(LHCb::ODIN {parameters.dev_odin_data[event_number]}.nonZeroSuppressionMode());
    if (nzst == nzs_bit) {
...
void odin_nzsfilter::odin_nzsfilter_t::operator()(
  ...
  const Allen::Context& context) const
{
  ...
  global_function(odin_nzsfilter_kernel)(dim3(1), dim3(m_block_dim_x), context)(
    arguments, size<dev_event_list_t>(arguments), m_nzs_bit.value());
