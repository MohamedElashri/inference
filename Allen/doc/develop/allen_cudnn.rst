...
* ``HandleProvider`` and ``get_thread_local_handle`` for cuDNN handles owned
  per CUDA stream and bound to it, with a lock-free per-thread fast path.
...
* ``DeviceWeights`` for namespaced process-lifetime or externally owned device
  weights.
* ``ConvDescriptors`` and ``ConvBiasReluGraph``, fixed-shape wrappers used by
  PVFinder's production UNet (see below). They predate the plan API and are
  kept because that path is validated and benchmarked on them.
...
``PoolingPlan`` describes generic NCHW pooling. The tested representative is
max pooling with typed ``Pooling2DShape``/``Pooling1DShape`` shapes and explicit precision
metadata:
...
PVFinder's maxpool stays on its local kernel. A trial of replacing it with
``PoolingPlan`` missed the FP32 PyTorch validation tolerance, so the generic
plan is available and tested but not used by PVFinder.
...
PVFinder's UNet (``pvfinder_unet``) is the production client of
``AllenCuDNN``. It is not yet built on the plan API; it uses the fixed-shape
wrappers, which carry its throughput tuning. The patterns it shares with the
recommendations above:

* all model state lives in one per-instance object, so two ``pvfinder_unet``
  instances with different ``weight_file`` values in one sequence run their own
  weights, descriptors and CUDA graphs
* weights go through ``WeightRegistry`` with keys namespaced by instance and
  weight file; the registry is locked once event processing starts
* cuDNN handles come from ``get_thread_local_handle`` (one handle per stream)
* descriptors are created once, under ``std::call_once``, from the first live
  handle

Its tuned choices, which a new client should re-measure rather than copy:

* ``ConvDescriptors`` pins ``IMPLICIT_GEMM`` and keeps per-thread workspaces;
  under many concurrent Allen streams this beat algorithms that need MB-scale
  workspace
* ``ConvBiasReluGraph`` fuses Conv+Bias+ReLU through the backend graph API
  (opt-in ``use_fused_cbr``), falling back to the separate bias/ReLU kernel when
  no engine supports the graph
* FP16 and BF16 are opt-in properties; per-thread CUDA graphs are optional

Moving the UNet onto ``FusedConvPlan``/``ForwardConvPlan``/``BackwardDataConvPlan``
is a separate change and must match the current path in validation
(``make -C weights validate``) and in throughput at the production point
(``-n 500 -m 500 -r 1000 -t 16``) before it replaces it.
