...
def calo_cosmics_lines(ecal_clusters):
    ...
    lines.append(
        line_maker(
            make_single_calo_cluster_line(ecal_clusters, "Hlt1SingleCaloCluster")
        )
    )
    lines.append(
        line_maker(
...
def scifi_cosmics_lines(seed_tracks):
    ...
    lines.append(line_maker(make_t_cosmic_line(seed_tracks, name="Hlt1TCosmic")))
...
def alignment_monitoring_lines(velo_tracks):
    lines = [
        line_maker(make_velo_micro_bias_line(velo_tracks, name="Hlt1VeloMicroBias"))
    ]

    with line_maker.bind(prefilter=[sd_error_filter()]):
        lines += [
            line_maker(make_passthrough_line(name="Hlt1ErrorBank", pre_scaler=0.01))
        ]
...
def setup_hlt1_node(enableRateValidator=True):
    ...
    line_algorithms = [
        tup[0] for tup in cosmics_lines + scifi_lines + muon_lines + monitoring_lines
    ]
    # List of line nodes, required to set up the CompositeNode
    line_nodes = [
        tup[1] for tup in cosmics_lines + scifi_lines + muon_lines + monitoring_lines
    ]
    ...
    hlt1_node = CompositeNode(
        "Cosmics", [lines, persistency_node], NodeLogic.NONLAZY_AND, force_order=True
    )
