...
def create_reduced_scifi_container(dev_used_scifi_hits):
    ...
    reduced_scifi_hit_container = {
        "host_number_of_scifi_hits": create_reduced_scifi_hit_container.host_number_of_scifi_hits_t,
        "dev_scifi_hits": create_reduced_scifi_hit_container.dev_scifi_hits_t,
        "dev_scifi_hit_offsets": create_reduced_scifi_hit_container.dev_scifi_hit_offsets_t,
    }
...
def combine_long_containers(long_tracks_0, long_tracks_1):
    ...
    combined_tracks = {
        "dev_offsets_long_tracks": combine_long_track_containers.dev_offset_tracks_output_t,
        "dev_multi_event_long_tracks_view": combine_long_track_containers.dev_multi_event_long_tracks_view_output_t,
        "dev_long_track_view": combine_long_track_containers.dev_long_track_view_output_t,
        "dev_multi_event_long_tracks_ptr": combine_long_track_containers.dev_multi_event_long_tracks_ptr_output_t,
        "host_number_of_reconstructed_scifi_tracks": combine_long_track_containers.host_number_of_reconstructed_scifi_tracks_output_t,
        "dev_scifi_states": combine_long_track_containers.dev_scifi_states_output_t,
        "velo_tracks": long_tracks_0["velo_tracks"],
        "velo_kalman_filter": long_tracks_0["velo_kalman_filter"],
    }

    return combined_tracks


def best_track_creator(
    with_ut=True, tracking_type=TrackingType.FORWARD_THEN_MATCHING, algorithm_name=""
):
    if algorithm_name != "":
    ...
    if tracking_type == TrackingType.FORWARD_THEN_MATCHING:
        ...
        reduced_ut_hit_container = (
            create_reduced_ut_container(
                decode_ut(), forward_tracks["dev_used_ut_hits_offsets"]
            )
            if with_ut
            else None
        )
        ...
        seeding_tracks = make_seeding_tracks(
            reduced_scifi_hit_container,
            seeding_xz_tracks,
            scifi_consolidate_seeds_name=algorithm_name + "scifi_consolidate_seeds",
        )
        matched_tracks = make_velo_scifi_matches(
            velo_tracks,
            velo_kalman_filter,
            seeding_tracks,
            accepted_velo_tracks=forward_tracks["dev_accepted_and_unused_velo_tracks"],
        ...
        forward_tracks = make_forward_tracks(
            ...
            with_ut,
            scifi_consolidate_tracks_name=algorithm_name + "scifi_consolidate_tracks",
        )
