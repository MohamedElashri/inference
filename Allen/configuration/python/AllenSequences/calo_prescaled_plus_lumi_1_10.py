    ...
    lines.append(
        line_maker(
            make_single_calo_cluster_line(
                ecal_clusters, name="Hlt1SingleCaloCluster", minEt=400, pre_scaler=0.10
            )
        )
    )
...
with line_maker.bind(prefilter=prefilters + [odin_lumi_event, odin_orbit]):
    lines += [line_maker(make_passthrough_line(name=lumilinefull_name, pre_scaler=1.0))]
...
lumi_with_prefilter = CompositeNode(
    "LumiWithPrefilter", prefilters + [lumi_node], NodeLogic.LAZY_AND, force_order=True
)

hlt1_node = CompositeNode(
    "AllenWithLumi",
    [calo_sequence, lumi_with_prefilter],
    NodeLogic.NONLAZY_AND,
    force_order=False,
)
