...
def generate_json_configuration(algorithms):
    ...
    configured_algorithms = [
        [algorithm.type.getName(), algorithm.name, algorithm.type.category()]
        for algorithm in algorithms
    ]
    ...
    for algorithm in algorithms:
        configured_arguments += [
            [algorithm.type.__slots__[a[0]].Scope, clean_prefix(a[1].location)]
    ...
    for algorithm in algorithms:
        ...
        gaudi_data_handles = [
            p
            for p in algorithm.type.getDefaultProperties().items()
        ...
        for (
            ...
            if (
                parameter_name in algorithm.inputs
                and type(algorithm.inputs[parameter_name]) == list
            ):
                input_aggregate = []
                for single_param_i, single_parameter in enumerate(
                    algorithm.inputs[parameter_name]
                ):
                    parameter_full_name = clean_prefix(single_parameter.location)
