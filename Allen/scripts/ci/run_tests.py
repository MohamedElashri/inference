...
def post_proc_throughput(
    ...
    if "throughput_report" in test and not test["throughput_report"]:
...
def build_allen_args(config, test, target):
    ...
    if "collision_type" in test:
        ...
        collision_type = "pp"
    if (
        test["type"] in target_args_cfg
        and target in target_args_cfg[test["type"]][collision_type]
    ):
...
def build_allen_env(config, test, target, **env_args):
    ...
    if target.endswith("PROF"):
        ...
        assert "RUN_PROFILER_OUTPUT" in env_args, (
            f"RUN_PROFILER_OUTPUT must be set for {target} target"
        )
...
def main():
    ...
    if args.test_group not in config or args.test_group == "config":
        raise ValueError(
