...
namespace Allen::Conditions {
  struct EcalGeometry {
    ...
    inline static std::string const filename = "ecal_geometry.bin";
#ifdef USE_DD4HEP
    inline static std::string const DefaultLocation = "/world:AllenConditions-ecal-geometry";
#else
    inline static std::string const DefaultLocation = "AllenConditions-ecal-geometry";
#endif
    ...
    static auto addConditionDerivation(PARENT* parent)
    {
      return parent->addConditionDerivation(
        {DeCalorimeterLocation::Ecal}, EcalGeometry::DefaultLocation, [](DeCalorimeter const& det) {
          return Allen::Conditions::EcalGeometry {det};
        });
    }

    EcalGeometry(const DeCalorimeter& det)
    {
    ...
    EcalGeometry(const std::vector<char>& data) { m_data = data; }

    void update_constants(Constants& constants) const
    {
      auto& dev_geometry = constants.dev_ecal_geometry;
      auto& host_geometry = constants.host_ecal_geometry;
