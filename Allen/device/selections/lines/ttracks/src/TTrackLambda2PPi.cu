...
\*****************************************************************************/
#include "TTrackLambda2PPi.cuh"
#include <ROOTHeaders.h>
#include "ROOTService.h"

INSTANTIATE_LINE(ttrack_lambda2ppi::ttrack_lambda2ppi_t, ttrack_lambda2ppi::Parameters)

__device__ bool ttrack_lambda2ppi::ttrack_lambda2ppi_t::select(
  const Parameters&,
  ...
  std::tuple<const Allen::Views::Physics::CompositeParticle, const unsigned> input)
{
  ...
  const auto massA = childA->state().p() > childB->state().p() ? Allen::mP : Allen::mPi;
  const auto massB = childA->state().p() > childB->state().p() ? Allen::mPi : Allen::mP;

  const auto assymetry = fabsf(childA->state().p() - childB->state().p()) / (childA->state().p() + childB->state().p());
  ...
  return (composite.charge() == 0) && (vertex.ttracks_doca() < props.max_doca) && (vertex.pt() > props.min_pt) &&
         ...
         (composite.maxp() > props.max_track_p) && (assymetry > props.min_assymetry) &&
         (vertex.ttracks_ip2() < props.min_ip2) && (composite.m12(massA, massB) < props.max_m) &&
         (!childA->is_lepton()) && (!childB->is_lepton()) && (composite.minp() > props.min_track_p) &&
         (vertex.z() < props.max_ovtx_z) && (vertex.ttracks_dira() > props.min_dira);
}

__device__ void ttrack_lambda2ppi::ttrack_lambda2ppi_t::monitor(
  const Parameters&,
  ...
  bool sel)
{
  if (sel) {
    ...
    const auto childA = static_cast<const Allen::Views::Physics::BasicParticle*>(part.child(0));
    const auto childB = static_cast<const Allen::Views::Physics::BasicParticle*>(part.child(1));
    const auto massA = childA->state().p() > childB->state().p() ? Allen::mP : Allen::mPi;
    const auto massB = childA->state().p() > childB->state().p() ? Allen::mPi : Allen::mP;

    properties.histogram_mass.increment(part.m12(massA, massB));
...
__device__ bool ttrack_lambda2ppi::ttrack_lambda2ppi_t::fill_tuples(
  ...
  bool sel)
{
  if (sel) {
    ...
    parameters.mon_mass[index] = part.m12(massA, massB);
