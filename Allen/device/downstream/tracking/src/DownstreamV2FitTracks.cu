...
// This
#include "DownstreamV2FitTracks.cuh"
...
INSTANTIATE_ALGORITHM(downstream_v2_fit_tracks::downstream_v2_fit_tracks_t);

void downstream_v2_fit_tracks::downstream_v2_fit_tracks_t::set_arguments_size(
  ArgumentReferences<Parameters> arguments,
  const RuntimeOptions&,
  const Constants&) const
{
  // Basics
  const auto number_of_tracks = first<host_number_of_downstream_compact_tracks_t>(arguments);
  const auto number_of_events = first<host_number_of_events_t>(arguments);

  // Set sizes
  set_size<host_number_of_fitted_downstream_compact_tracks_t>(arguments, 1);
  set_size<dev_fitted_downstream_compact_track_offsets_t>(arguments, number_of_events + 1);
  set_size<dev_fitted_downstream_compact_tracks_t>(arguments, number_of_tracks);
  set_size<dev_fitted_downstream_compact_track_scores_t>(arguments, number_of_tracks);
}

void downstream_v2_fit_tracks::downstream_v2_fit_tracks_t::operator()(
  ...
  const Allen::Context& context) const
{
  ...
  Allen::memset_async<dev_fitted_downstream_compact_track_offsets_t>(arguments, 0, context);

  global_function(downstream_v2_fit_tracks)(dim3(size<dev_event_list_t>(arguments)), m_block_dim, context)(
    arguments, m_downstream_clone_killer.getDevicePointer(), m_clone_killing_threshold);

  PrefixSum::
    prefix_sum<dev_fitted_downstream_compact_track_offsets_t, host_number_of_fitted_downstream_compact_tracks_t>(
      *this, arguments, context);
}

__global__ void downstream_v2_fit_tracks::downstream_v2_fit_tracks(
  Parameters parameters,
  const CloneKiller::DeviceType* clone_killer,
  float clone_killing_threshold)
{
  // Basics
  ...
  // Downstream Input
  const auto offset = parameters.dev_downstream_compact_track_offsets[event_number];
  const auto input_size = parameters.dev_downstream_compact_track_offsets[event_number + 1] - offset;
  const auto input_tracks = parameters.dev_downstream_compact_tracks + offset;

  // Output
  auto output_counter = parameters.dev_fitted_downstream_compact_track_offsets + event_number;
  auto output_tracks = parameters.dev_fitted_downstream_compact_tracks + offset;
  auto output_scores = parameters.dev_fitted_downstream_compact_track_scores + offset;
  ...
  for (unsigned track_idx = threadIdx.x; track_idx < input_size; track_idx += blockDim.x) {
    ...
    if (!std::isfinite(score) || score > clone_killing_threshold) continue;
