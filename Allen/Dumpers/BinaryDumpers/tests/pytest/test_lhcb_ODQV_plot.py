...
class Test(LHCbExeTest):
    ...
    def test_stderr(self, stderr: bytes):
        stderr_preprocessor = LineSkipper(
            regexps=[
    ...
    def test_expected_files(self, cwd: Path):
        ...
        expectedFiles = [
            "fileCanvas.pdf",
            "IPforwardCanvas.pdf",
            "IPmatchingCanvas.pdf",
            "kalmanCovCanvas.pdf",
            "longForwardCanvas.pdf",
            "longMatchingCanvas.pdf",
            "occupancyCanvas.pdf",
            "PIDCanvas.pdf",
            "PIDkinCanvas.pdf",
            "PVcanvas.pdf",
            "PVcovCanvas.pdf",
            "PVdistCanvas.pdf",
            "IPresolutionCanvas.pdf",
            "veloCanvas.pdf",
        ]

        for file in expectedFiles:
            assert os.path.exists(cwd / file), (
                f"could not find expected output file: {file} \n"
            )
