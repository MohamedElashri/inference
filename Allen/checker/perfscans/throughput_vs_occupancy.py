...
for ibin in range(n_bins):
    ...
    if n_events > 1000:
        ...
        proc = subprocess.Popen(
            ["tail", "-n", "2", stdout_file], stdout=subprocess.PIPE
        )
        for line in proc.stdout:
...
for labelx, labely, labelsecy in zip(
    ax.get_xticklabels(), ax.get_yticklabels(), secaxy.get_yticklabels()
):
...
legend = plt.legend(
    ["Data volume distribution", "Throughput"],
    frameon=False,
    prop=legend_font,
    loc="lower left",
)
