...
scans = {
    ...
    "PV": {"zmin": [], "zmax": [], "maxChi2": [], "minTracksInSeed": []},
    "CompassUT": {
        ...
        for entry in files:
            if fnmatch.fnmatch(entry, pattern):
                scans[thisscan][var].append(
                    entry.lstrip(thisscan)
                    .lstrip("-")
                    .lstrip(var)[1:]
                    .rstrip("-" + physperfsuffix)
                )
...
for thisscan in scanstoplot:
    ...
    for var in scans[thisscan]:
        ...
        if scans[thisscan][var][0][0] == "-":
            ...
            lowlim /= 1.05
            upplim *= 1.05
        tpthistos[thisscan][var] = TH1F(
            thisscan + var + "tphist", thisscan + var + "tphist", 100, lowlim, upplim
        )
                ...
                if line.find("events/s") > -1:
                    if firstpoint:
                        ...
                        tpthistos[thisscan][var].Fill(
                            float(scanpoint), float(line.split()[0]) / relfactor
                        )
    ...
    for var in scans[thisscan]:
        ...
        for ppgroup in ordertoread:
            for ppcat in physperftoplot[ppgroup]["cats"]:
                ...
                physperfhistos[thisscan][var][ppgroup + ppcat].GetYaxis().SetTitle(
                    "Efficiency (%)"
                )
                physperfhistos[thisscan][var][
                    ppgroup + ppcat
                ].GetYaxis().SetTitleOffset(0.9)
                physperfhistos[thisscan][var][ppgroup + ppcat].GetXaxis().SetTitle(var)
                physperfhistos[thisscan][var][ppgroup + ppcat].GetXaxis().SetTitleSize(
                    0.05
                )
                physperfhistos[thisscan][var][
                    ppgroup + ppcat
                        ...
                        if line.find(ppcat) > -1:
                            if ppcat == "ghosts":
                                if line.find("TrackChecker output") == -1:
                                    continue
                            if ppgroup == "PV":
                                effval = 100.0 * float(line.split(":")[1].split("(")[0])
                            else:
                                if ppcat == "ghosts":
                                    effval = float(
                            ...
                            physperfhistos[thisscan][var][ppgroup + ppcat].Fill(
                                float(scanpoint), effval
                            )
        ...
        for ppgroup in ordertoread:
            ...
            for col, ppcat in enumerate(physperftoplot[ppgroup]["cats"]):
                ...
                if firstcat:
                    ...
                    physperfhistos[thisscan][var][
                        ppgroup + ppcat
                    ].GetYaxis().SetRangeUser(0, 100)
                    firstcat = False
                else:
                    physperfhistos[thisscan][var][ppgroup + ppcat].Draw("PSAME")
                if ppgroup == "PV":
                    legendpv.AddEntry(
                        physperfhistos[thisscan][var][ppgroup + ppcat], ppcat, "p"
                    )
                else:
                    legend.AddEntry(
                        physperfhistos[thisscan][var][ppgroup + ppcat], ppcat, "p"
            ...
            canvtoploton.SaveAs(
                resultsdir + thisscan + "-" + var + "-" + ppgroup + "-ppscan.pdf"
            )
            legcanv.cd()
            if ppgroup == "PV":
                legendpv.Draw()
                ...
                legend.Draw()
            legcanv.SaveAs(
                resultsdir + thisscan + "-" + var + "-" + ppgroup + "-legend.pdf"
            )
