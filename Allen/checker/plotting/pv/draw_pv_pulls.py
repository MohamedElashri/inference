...
reshist = {}
for coord in ["x", "y", "z"]:
    reshist[coord] = ROOT.TH1F(
        "pullhist" + coord, "pullhist" + coord, 50, -1.0 * xrange, xrange
    )
...
for coord in ["x", "y", "z"]:
    ...
    pvcanv[coord].cd(1)
    func[coord] = ROOT.TF1("gausx", "gaus(0)", -1.0 * xrange, xrange)
    reshist[coord].Fit(func[coord])
    reshist[coord].GetXaxis().SetTitle(
        "#frac{#delta_{" + coord + "}}{#sigma_{" + coord + "}}"
    )
    reshist[coord].GetYaxis().SetTitle("Entries")
    reshist[coord].GetYaxis().SetTitleOffset(1.2)
    reshist[coord].GetXaxis().SetTitleOffset(1.0)
    reshist[coord].Draw()
