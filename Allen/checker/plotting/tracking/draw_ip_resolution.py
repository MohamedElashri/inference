...
def ipRes3D(tree, var, ptBin):
    ...
    tree.Draw(
        var + ">>h",
        "ghost==0 && 1000./best_pt>={} && 1000./best_pt<{}".format(ptBin[0], ptBin[1]),
        "goff",
    )
...
def makeGraph(tree, var, resFunc):
    ...
    g = ROOT.TGraphErrors(
        len(centers),
        centers,
        array("d", 1000.0 * res[:, 0]),
        widths,
        array("d", 1000.0 * res[:, 1]),
    )
...
if __name__ == "__main__":
    ...
    for info in plotInfo:
        ...
        txtVelo = (
            "#sigma_{VELO} = "
            + "({:2.1f}#pm{:2.1f}) + ".format(
                fVelo.GetParameter(0), fVelo.GetParError(0)
            )
            + "({:2.1f}#pm{:2.1f})".format(fVelo.GetParameter(1), fVelo.GetParError(1))
            + "/#it{p}_{T} #mum"
        )
        txtKalman = (
            "#sigma_{Kalman} = "
            + "({:2.1f}#pm{:2.1f}) + ".format(
                fKalman.GetParameter(0), fKalman.GetParError(0)
            )
            + "({:2.1f}#pm{:2.1f})".format(fVelo.GetParameter(1), fVelo.GetParError(1))
            + "/#it{p}_{T} #mum"
        )
